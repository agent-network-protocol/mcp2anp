...
"""
端到端 (E2E) 测试：直接连接到已运行的 HTTP 服务器 (0.0.0.0:9881)。
不使用环境变量，全部硬编码。

覆盖：
1) 有效密钥 -> /anp.fetchDoc 放行 (HTTP 200)
2) 缺失密钥 -> 401
3) 无效密钥 -> 401
4) 非法请求体验证 -> 422
5) （可选）/anp.invokeOpenRPC 成功路径（默认跳过，依赖外部 OpenRPC 端点）
"""
...
# ---------------------- 硬编码配置
BASE_URL = "https://agent-connect.ai/http2anp"  # 已运行的后端地址

# BASE_URL = "http://localhost:9881"
API_KEY_HEADER = "X-API-Key"
VALID_KEY = "sk_mcp_6OIGY-42ZVG-4H2NU-CCI3L-IVHUV-6XWTI-5CY7I-K4CDW-54"
...
def _server_reachable(base_url):
    ...
    首选探测 /openapi.json；若不存在则尝试 OPTIONS /anp.fetchDoc。
    """
    try:
